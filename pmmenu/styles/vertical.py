from pmmenu.menuitem import *
from pmmenu.pmutil import *
from pmmenu.pmheader import *
import thread, time

class PMVertical(pygame.sprite.OrderedUpdates):
	...
	def __init__(self, menu_item_cfgs, cfg):
		...
		self.header = pygame.sprite.RenderPlain((PMHeader(self.cfg.options,  logo_align=('left', 'top'), ip_align = ('right','top'), update_align = ('right', 'top'))))
	...
	def set_position(self, position = 0):
		...
		spacing = self.sprites()[0].rect.h + self.cfg.options.padding
		
		screen_center = (pygame.display.Info().current_w / 2, 
								((pygame.display.Info().current_h - self.cfg.options.header_height) / 2) +
								self.cfg.options.header_height)

		self.current_position = position
		x = max(item.rect.w for item in self.sprites()) / 2 + self.cfg.options.padding
		y = screen_center[1] - (spacing * position)
	...
	def ease(self, current_time, start_value, change_in_value, duration):
		...
		#out
		current_time -= 1
		return -change_in_value / 2 * (current_time * (current_time - 2) - 1) + start_value - (change_in_value % 2)
	...
	def draw_items(self, new_position = 0):
		...
		spacing = self.sprites()[0].rect.h + self.cfg.options.padding
		distance = (self.current_position - self.new_position) * spacing
		duration = 25
		
		start = self.sprites()[0].rect.copy()
		offset = 0
		for x in xrange(1, duration+1):
			for sprite in self.sprites():
				sprite.rect.move_ip(0, self.ease(x, 0, distance, duration) - self.ease(x-1, 0, distance, duration))
				if x == duration:
					if sprite is self.sprites()[0]:
						if sprite.rect.y - start.y != distance:
							offset = distance - (sprite.rect.y - start.y)
					sprite.rect.move_ip(0, offset)
			
			self.clear(self.screen, self.cfg.options.pre_loaded_background)
			self.update_display = self.draw(self.screen)
			self.header.draw(self.screen)
		...
		if render_now:
			self.clear(self.screen, self.cfg.options.pre_loaded_background)
			self.update_display.extend(self.draw(self.screen))
			self.header.draw(self.screen)
			#if self.boxart_on_screen: 
			#	self.draw_bg(self.info_container)
			#self.data_thread = thread.start_new_thread(self.draw_data, (5,))
		...
		def update(self, menu_item):
			...
			if menu_item.icon_selected:
				...
				if menu_item.display_label:
					label = PMLabel(menu_item.label, self.cfg.options.label_font, self.cfg.options.label_font_selected_color, self.cfg.options.label_background_selected_color, self.cfg.options.label_font_selected_bold, self.cfg.options.label_max_text_width, outline_color = self.cfg.options.label_font_outline_selected_color)
					textpos = label.rect
